...
- **Find Next** — Return in the query field, or **Cmd+G** — steps to the next match and wraps past the last one back to the first. **Find Previous** — **Cmd+Shift+G** — steps to the previous match and wraps past the first one back to the last; it has **no Return-key equivalent**, and specifically **Shift+Return in the query field does not step backwards** — SwiftUI's `.onSubmit` carries no modifier information, so the field cannot distinguish Shift+Return from Return and treats it as an ordinary submit (i.e. a *forward* step, not a backward one). Users reaching for the browser/Xcode ⇧↩ reflex must use Cmd+Shift+G. Making ⇧↩ work would require replacing the query field with an `NSViewRepresentable` or installing a key monitor, which is deliberately out of scope. A step scrolls but does **not** move the caret; **closing** the bar (Esc, or Done) places the caret at the current match once and removes every highlight. Esc closes the bar whether focus is in the query field or in the editor text.
...
- Horizontal rules (`---`, `***`).
- Inline: `**bold**`, `*italic*`, `` `code` ``, `[title](url)` (styled as link; clickable). Emphasis follows **CommonMark 0.30's delimiter-run and delimiter-stack rules**, and the subset adopted is exactly: maximal runs of `*` only (`_` is not an emphasis delimiter here); a run may open emphasis only if it is *left-flanking* and close only if it is *right-flanking*, where flanking is judged against CommonMark's own whitespace (Unicode Zs plus tab, LF, FF, CR) and punctuation (Unicode Pc/Pd/Pe/Pf/Pi/Po/Ps plus the ASCII punctuation characters, nine of which — ``$ + < = > ^ ` | ~`` — are outside those categories) and the start and end of a block count as whitespace; two asterisks from each side make bold and one makes italic, so emphasis **nests** (`*a **b** c*` is one italic containing a bold); a pair is refused when either run can both open and close and the sum of the two runs' original lengths is a multiple of three unless both are ("the rule of three"); and a run that matches nothing renders as literal asterisks, so **no asterisk is ever deleted**. Consequences worth stating: `***x***` is italic-around-bold; a run that pairs with nothing is literal text, so `a****b` renders as `a****b` (a *whole line* of `****` never reaches the inline parser at all — three or more `*` alone on a line is a horizontal rule, per the bullet above); a stray `*` (`2 * 3 and **bold** here`) no longer re-pairs every delimiter after it; and **code spans and links resolve over the whole block rather than inside an emphasis body**, so `` *a`b* c`d `` is a code span spanning the `*` and `*a [b* c](d)` is a real link — text that was previously plain can therefore become a clickable link. **That last case is a deliberate, user-visible reversal:** before this change a `[…](…)` whose brackets straddled an emphasis delimiter rendered as plain literal text, and it now renders as a **live, clickable link** whose URL is no longer shown as text. Rendered output can therefore be *shorter* than it was for such inputs — the URL moved into a link attribute rather than being lost. **Emphasis nesting is capped at 32 levels** — the same convention §5.2 and the table cell budget follow, bound the derived structure and declare it: real Markdown nests two or three deep, while an uncapped tree is unbounded in the input and every consumer of it recurses per level, which made a 1.9 kB document crash the background render. A pair that would exceed the cap does not form and its delimiters render literally, so no character is lost. This is not full CommonMark: no `_` emphasis, no backslash escapes, no HTML, no reference links, code spans still pair backticks 1-2, 3-4, … rather than by run length, and delimiter runs are matched over Swift *grapheme clusters* rather than Unicode scalars — so an asterisk carrying a combining mark (`*` + U+0301) is one cluster, is not equal to `*`, and is therefore not a delimiter at all, where a scalar-based implementation would see one. The editor's syntax highlighting (§6.3) has no flanking rule at all, so editor and preview deliberately disagree about which asterisks are delimiters.
- **GFM pipe tables**, rendered as a real `NSTextTable` grid — column widths come from the layout engine, so a wide table fits the preview's narrow column by wrapping *inside* a cell rather than overflowing (the preview has no horizontal scroller). The supported subset, exactly: a **header row and a delimiter row are both required** — a `|` line with no delimiter row under it is ordinary paragraph text — and **the delimiter row must itself contain a `|`**, so a bare `---` following a line that happens to contain a pipe is still a horizontal rule. Leading and trailing pipes are optional (`a | b` works); `:---` / `---:` / `:---:` set left / right / centre alignment per column; inline spans (`**bold**`, `*italic*`, `` `code` ``, links) are parsed inside cells, and a `|` inside a matched pair of backticks does not split a cell. **The header fixes the column count**: a short row is padded with empty cells and a long row's surplus cells are re-joined into its last cell with their `|` separators restored, so ragged input degrades without losing a single character. The header row renders bold; the delimiter row renders nothing. A table under an indented bullet is a table, not part of the bullet. A `|` line that does **not** qualify as a table — no delimiter row after it — is likewise not absorbed into a bullet above it: it ends the item and renders as paragraph text, exactly as it did before tables existed. **A table is capped at 50,000 cells** (`columns × (rows + 1)`, the same declared budget §5.2 uses for a scanned tree, and for the same reason): the header alone fixes the column count and short rows are padded, so `columns × rows` cells can be produced from `columns + rows` bytes of source — a 3 kB document can otherwise reach a million cells and near a gigabyte of transient memory. A construct over the cap is **not** a table and renders as ordinary paragraph text, which is what it did before tables were supported. Within the cap the grid costs roughly 370 bytes of retained storage per cell (one `NSTextTableBlock` and one paragraph style each, and adjacent cells' styles cannot coalesce), so a table at the cap adds ~18 MB — bounded, but a real addition to the §1 band, and the reason the cap is not higher. Not supported: escaped pipes (`\|`), block content or multiple lines inside a cell, row/column spans, nested tables. The editor's syntax highlighting (§6.3) has **no** table rule, so editor and preview deliberately disagree about tables.
- Not in v1: images, footnotes, HTML passthrough, nested lists beyond one level, setext headings.
...
- A **table emits exactly one anchor**, at its header row — not one per row. Scrolling through a twenty-row table therefore moves the preview only once, when the editor's first visible line enters or leaves the table: the same trade fenced code blocks already make, and the price of the block-level anchor model.
...
| Edit → Find Previous | Cmd+Shift+G | step to the previous match, wrapping at the start (§6.5); disabled when no file is open |

Commands act on the focused window's state (`focusedSceneObject`), except **Open Folder… (Cmd+O)**, which is app-level — it creates a new window and is not focused-window-scoped. **New… (Cmd+N)** is focused-window-scoped: it presents its sheet in the key window and creates in that window's target directory only. **Find / Find Next / Find Previous (§6.5)** are focused-window-scoped as well, and being menu key equivalents is what lets Cmd+F reach the editor's find bar even while the sidebar filter field has focus.
...
10. In-editor find (§6.5) — Cmd+F / Cmd+G, a visible case-sensitivity checkbox, layout-manager highlights, which **reversed** the `find/replace` non-goal of §12 down to replace only. Find Previous was explicitly *not* part of this step (see DONE.md's record for `(editor-find)`); it arrived in step 11.
11. Follow-on work on find and the Markdown preview: Find Previous (Cmd+Shift+G, §6.5/§10).
